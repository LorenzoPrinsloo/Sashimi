package roflsoft.model.request

import octopus.dsl._

case class UserRegisterRequest(email: String, password: String)
object UserRegisterRequest {

  implicit val userRegisterValidator: Validator[UserLoginRequest] = Validator[UserLoginRequest]
    .rule(request => isValidEmail(request.emailAddress), "Invalid email address.")
    .rule(request => isValidPassword(request.password), "Password must be at least 10 characters long, including an uppercase and a lowercase letter, one number and one special character.")

  private def isValidEmail(email: String): Boolean = {
    email.matches("(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|\"(?:[\\x01-\\x08\\x0b\\x0c\\x0e-\\x1f\\x21\\x23-\\x5b\\x5d-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])*\")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\\x01-\\x08\\x0b\\x0c\\x0e-\\x1f\\x21-\\x5a\\x53-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])+)\\])")
  }

  private def isValidPassword(password: String): Boolean = {
    password.matches("(?=^.{10,}$)((?=.*\\d)|(?=.*\\W+))(?![.\\n])(?=.*[A-Z])(?=.*[a-z]).*$")
  }

}
